...
pub fn register<F: FnMut(String) + Send + 'static>(scheme: &str, handler: F) -> Result<()> {
	...
	let exe = crate::STARTING_BINARY.cloned()
		.unwrap()
		.display()
		.to_string()
		.replace("\\\\?\\", "");
...
fn dummy_keypress() {
	...
	keyboard_input_up.dwFlags = 0x0002;

	let input_down_u = INPUT_0 { ki: keyboard_input_down };
	let input_up_u = INPUT_0 {  ki: keyboard_input_up };
